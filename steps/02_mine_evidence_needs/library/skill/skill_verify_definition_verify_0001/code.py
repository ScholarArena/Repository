...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if not term:
            return runtime.missing(reason="missing_term", obs_type="verification_result")
        prim, err = _require_primitive(primitives, "Extract_Definition")
        if err:
            return err
        obs = prim(context, {"term": term, "max_matches": params.get("max_matches")})
        if obs.get("status") != "ok":
            return runtime.missing(reason="definition_not_found", payload={"term": term}, prov=obs.get("prov", []), obs_type="verification_result")
        texts = _extract_match_texts(obs)
        payload = {
            "term": term,
            "supported": True,
            "matches": (obs.get("payload") or {}).get("matches", []),
            "evidence_texts": _truncate_texts(texts, int(params.get("max_chars") or 1000)),
        }
