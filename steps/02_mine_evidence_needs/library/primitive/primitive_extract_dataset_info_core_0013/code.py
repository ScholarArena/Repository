...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        keywords = ["dataset", "benchmark", "corpus", "data set"]
        def matcher(text):
            lower = text.lower()
            if dataset:
                return dataset if dataset.lower() in lower else ""
            return "dataset" if any(key in lower for key in keywords) else ""
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"dataset": dataset})
        if matches:
