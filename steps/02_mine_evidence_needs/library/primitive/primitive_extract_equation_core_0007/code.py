...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        eq_id = _normalize_eq_id(params.get("eq_id"))
        if not eq_id:
            return runtime.missing(error="missing_eq_id")
        pat = re.compile(r"\b(?:Eq\.?,?|Equation)\s*\(?" + re.escape(eq_id) + r"\)?\b", re.IGNORECASE)
        def matcher(text):
            return eq_id if pat.search(text) else ""
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"eq_id": eq_id})
        if matches:
