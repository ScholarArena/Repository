...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        algo_id = _normalize_algo_id(params.get("algo_id"))
        if not algo_id:
            return runtime.missing(error="missing_algo_id")
        pat = re.compile(r"\b(?:Algorithm|Alg\.?)\s*" + re.escape(algo_id) + r"\b", re.IGNORECASE)
        def matcher(text):
            return algo_id if pat.search(text) else ""
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"algo_id": algo_id})
        if matches:
