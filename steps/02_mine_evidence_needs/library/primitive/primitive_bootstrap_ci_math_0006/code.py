...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        values = _to_float_list(params.get("values"))
        if len(values) < 2:
            return runtime.missing(reason="insufficient_samples", obs_type="bootstrap_ci")
        n_boot = int(params.get("n_boot") or 1000)
        ...
        if np is not None:
            ...
            for _ in range(n_boot):
                ...
                sample = arr[idx]
                if stat == "median":
                    samples.append(float(np.median(sample)))
                else:
            ...
            for _ in range(n_boot):
                ...
                if stat == "median":
                    ...
                    if len(sample_sorted) % 2 == 0:
                        value = (sample_sorted[mid - 1] + sample_sorted[mid]) / 2
                    else:
                        value = sample_sorted[mid]
                else:
                    value = sum(sample) / len(sample)
                samples.append(value)
            samples.sort()
            lower_idx = int(max(0, (alpha / 2) * n_boot))
            upper_idx = int(min(n_boot - 1, (1 - alpha / 2) * n_boot))
        ...
        return runtime.ok(payload=payload, prov=[], obs_type="bootstrap_ci")
    except Exception as exc:
        return runtime.fail(error=str(exc), obs_type="bootstrap_ci")
