...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if references:
            return runtime.ok(payload=out_payload, prov=[], obs_type="external_references")
        return runtime.missing(reason="no_references", payload=out_payload, prov=[], obs_type="external_references")
    except Exception as exc:
        return runtime.fail(error=str(exc), obs_type="external_references")
