...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        keywords = ["train", "optimizer", "epoch", "schedule", "early stopping", "loss"]
        def matcher(text):
            lower = text.lower()
            if re.search(r"\bno\s+train(ing)?\b", lower):
                return ""
            return "training" if any(k in lower for k in keywords) else ""
