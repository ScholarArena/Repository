...
def _notation_matcher(symbol):
    ...
    def matcher(text):
        if sym not in text:
            return ""
        lower = text.lower()
        if "denote" in lower or "represent" in lower or " is " in lower or " are " in lower:
            return symbol
        return ""
    return matcher
...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if not symbol:
            return runtime.missing(error="missing_symbol")
        matcher = _notation_matcher(symbol)
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"symbol": symbol})
        if matches:
