...
def _subsection_matcher(title):
    if not title:
        return None
    title_lower = title.lower()
    pat = re.compile(r"^\s*\d+\.\d+\s+" + re.escape(title) + r"\b", re.IGNORECASE)
    def matcher(text):
        if pat.search(text):
            return title
...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if not title:
            return runtime.missing(error="missing_title")
        max_matches = int(params.get("max_matches") or 5)
        if max_matches <= 0:
            max_matches = 5
        matcher = _subsection_matcher(title)
        matches = _make_matches(segments, matcher, max_matches)
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"title": title})
        if matches:
