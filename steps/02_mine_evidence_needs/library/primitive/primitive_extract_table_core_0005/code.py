...
def _normalize_table_id(value):
    digits = re.findall(r"\d+", str(value or ""))
    return digits[0] if digits else str(value or "").strip()
...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        table_id = _normalize_table_id(params.get("table_id"))
        if not table_id:
            return runtime.missing(error="missing_table_id")
        pat = re.compile(r"\b(?:Table|Tab\.?)\s*" + re.escape(table_id) + r"\b", re.IGNORECASE)
        def matcher(text):
            return table_id if pat.search(text) else ""
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"table_id": table_id})
        if matches:
