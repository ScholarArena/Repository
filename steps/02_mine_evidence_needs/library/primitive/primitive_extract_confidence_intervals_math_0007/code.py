...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if not text:
            return runtime.missing(reason="missing_text", obs_type="confidence_interval")
        intervals = []
        for match in re.findall(r"\[\s*([-+]?\d*\.?\d+)\s*,\s*([-+]?\d*\.?\d+)\s*\]", text):
            intervals.append({"lower": float(match[0]), "upper": float(match[1]), "pattern": "bracket"})
        for match in re.findall(r"([-+]?\d*\.?\d+)\s*\u00b1\s*([-+]?\d*\.?\d+)", text):
            center = float(match[0])
            half = float(match[1])
            intervals.append({"lower": center - half, "upper": center + half, "pattern": "plus_minus"})
        payload = {"intervals": intervals, "text": text}
        if intervals:
            return runtime.ok(payload=payload, prov=[], obs_type="confidence_interval")
        return runtime.missing(reason="no_intervals", payload=payload, prov=[], obs_type="confidence_interval")
    except Exception as exc:
        return runtime.fail(error=str(exc), obs_type="confidence_interval")
