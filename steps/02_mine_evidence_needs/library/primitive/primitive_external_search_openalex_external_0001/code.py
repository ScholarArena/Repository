...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if err_code == "not_found":
            return runtime.missing(reason="not_found", payload={"query": query}, prov=[])
        if err_code:
        ...
        out_payload = {
            "query": query,
            "count": (payload.get("meta") or {}).get("count") if isinstance(payload, dict) else 0,
            "results": compact,
            "sources": _source_payload(source_meta),
        }
        if compact:
            return runtime.ok(payload=out_payload, prov=[], obs_type="external_search")
        return runtime.missing(reason="no_results", payload=out_payload, prov=[], obs_type="external_search")
    except Exception as exc:
        return runtime.fail(error=str(exc), obs_type="external_search")
