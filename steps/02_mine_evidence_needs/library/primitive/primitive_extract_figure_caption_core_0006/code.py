...
def _normalize_figure_id(value):
...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        fig_id = _normalize_figure_id(params.get("figure_id"))
        if not fig_id:
            return runtime.missing(error="missing_figure_id")
        pat = re.compile(r"\b(?:Figure|Fig\.?)\s*" + re.escape(fig_id) + r"\b", re.IGNORECASE)
        def matcher(text):
            return fig_id if pat.search(text) else ""
        matches = _make_matches(segments, matcher, int(params.get("max_matches") or 5))
        prov = _collect_prov(matches)
        payload = _payload_with_matches(matches, {"figure_id": fig_id})
        if matches:
