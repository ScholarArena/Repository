...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        ...
        if len(group_a) < 2 or len(group_b) < 2:
            return runtime.missing(reason="insufficient_samples", obs_type="t_test")
        paired = bool(params.get("paired"))
        equal_var = bool(params.get("equal_var"))
        if scipy_stats is not None:
            if paired:
                result = scipy_stats.ttest_rel(group_a, group_b, nan_policy="omit")
            else:
                result = scipy_stats.ttest_ind(group_a, group_b, equal_var=equal_var, nan_policy="omit")
            ...
            if paired:
                ...
                mean_d = _mean(diffs)
                var_d = _variance(diffs, ddof=1)
                t_stat = mean_d / math.sqrt(var_d / max(n_a, 1))
                df = n_a - 1
            elif equal_var:
                pooled = ((n_a - 1) * var_a + (n_b - 1) * var_b) / max(n_a + n_b - 2, 1)
                t_stat = (mean_a - mean_b) / math.sqrt(pooled * (1 / n_a + 1 / n_b))
                df = n_a + n_b - 2
            else:
        ...
        payload = {
            "t_stat": round(t_stat, precision),
            "p_value": round(p_val, precision),
            "method": method,
            "df": df,
        }
        return runtime.ok(payload=payload, prov=[], obs_type="t_test")
    except Exception as exc:
        return runtime.fail(error=str(exc), obs_type="t_test")
