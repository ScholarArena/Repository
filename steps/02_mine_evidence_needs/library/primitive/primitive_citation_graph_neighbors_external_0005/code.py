...
def _resolve_endpoint(params):
    ...
    if doi:
        doi = str(doi).strip()
        return urlparse.quote("https://doi.org/" + doi)
    return ""
...
def execute(context, params, primitives=None, controlled_llm=None):
    try:
        params = _normalize_params(params)
        direction = str(params.get("direction") or "cited_by").strip()
        key = _resolve_endpoint(params)
        if not key:
            return runtime.missing(error="missing_identifier")
        mailto = params.get("mailto") or os.environ.get("OPENALEX_MAILTO")
        if direction == "references":
            url = _build_url(f"works/{key}", {"mailto": mailto})
            ...
            references = payload.get("referenced_works") if isinstance(payload, dict) else []
            out_payload = {
                "direction": direction,
                "neighbors": references or [],
                "count": len(references or []),
                "sources": _source_payload(source_meta),
            }
            if references:
                return runtime.ok(payload=out_payload, prov=[], obs_type="external_citations")
            return runtime.missing(reason="no_neighbors", payload=out_payload, prov=[], obs_type="external_citations")
        ...
        out_payload = {
            "direction": direction,
            "neighbors": [item.get("id") for item in compact if item.get("id")],
            "count": len(compact),
            "results": compact,
            "sources": _source_payload(source_meta),
        }
        ...
        return runtime.fail(error=str(exc), obs_type="external_citations")
