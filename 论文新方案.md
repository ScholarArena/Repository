...
Skills 由离线 Compiler LLM 提出候选，但仅当 {spec, code, tests} 在沙箱中通过门控后才进入能力库；在线仅可调用已入库能力。
...
[
S_i^t = \big(\text{issue_tag},\ \text{ledger}_i^t,\ \text{budget}_i^t\big)
]
...
这保证评审交互在有限步内收敛，且便于评测与数据构建。所有非缺失 Observation 必须来自（i）确定性抽取 primitives 或（ii）tests-gated skills 的执行结果；不允许基于 LLM 虚构观测。
...
[
(\text{intent},\ \text{skill_call}) \sim \pi_\theta(\cdot \mid r,\ S_i^t,\ \mathcal{C})
]
[
...
[
S_i^t \xrightarrow{\pi_\theta} (\text{intent}_t,\ \text{skill_call}*t)
\xrightarrow{\text{Executor}} o_t
\xrightarrow{\pi*\theta} y_t
...
[
\mathcal{T}*t \subseteq {i} \sim \pi*\theta(\cdot \mid r=\text{Meta},\ {S_i^t},\ \mathcal{C})
]
...
1. **Move Planning**：((r, S_i^t, \mathcal{C}) \to (\text{intent}_t,\ \text{skill_call}_t))
...
如果你下一步要“更像论文”的版本（加入符号化的损失函数、训练细节、以及一个简洁算法框架 Algorithm 1 的伪代码），我可以在你确认想投的篇幅/风格（8页主文还是更长）后，直接把这一版改写成论文段落格式与算法框。


---

## 补充：Foundry 中的离线能力编译（Primitives/Skills Compilation）

### 总：动机与边界（与 Evidence-Conditioned Strategic Acts 对齐）

为实现 **证据条件化策略行为**，在线策略模型 (\pi_\theta) 必须将关键发言绑定到可核验观测 (o)。因此，系统需要一个可复现的“观测生成机制”。我们将其严格限制为：**在线仅调用已编译且测试通过的能力**；任何能力的生成、代码变更与测试迭代仅发生在离线 Foundry。

> 换言之：在线 Arena 学习 “如何在证据约束下行动”；离线 Foundry 负责 “如何可靠地产生证据”。

---

### 分：核心定义（与你给的 P/C/S/Compiler LLM 一致，且最小化）

#### 1) Primitive（(P)）：最小证据算子（不含 LLM）

[
p \in \mathcal{P}: \quad p(\mathcal{C}, \phi) \rightarrow o
]

* 目标：将常见、可重复的证据操作封装为最小单元（例如表格抽取、数值一致性检查、引用匹配、指标重算等）。
* 约束：**确定性/可测试**；输出 Observation schema；失败返回结构化 failure_code。

#### 2) Controlled-LLM Module（(C)）：受控 LLM 子模块（可选、强约束）

[
c \in \mathcal{C}!M: \quad c(z) \rightarrow z'
]

* 仅允许承担 **结构化抽取/判别/校验**（例如公式一致性判别、对齐表格字段归一化、离散 verdict）。
* 关键约束（写进论文作为安全与可复现条款）：

  * 输入只读 **结构化中间结果** 或局部引用片段（不可读整篇自由文本）；
  * 输出必须 JSON-only 且过 schema；
  * 温度 (T=0)，并纳入回归测试；
  * 其输出只能作为 Observation 的一部分（或用于生成派生字段），不能直接产生无证据主张。

> 这样 (C) 不会破坏“证据门控”的逻辑，而是作为“证据结构化/判别”的受控组件被测试集约束。

#### 3) Skill（(S)）：确定性编排模板（允许插入少量 (C) 节点）

[
s \in \mathcal{S}: \quad s(\mathcal{C}, \phi) \Rightarrow \text{DAG}(\mathcal{P}, \mathcal{C}!M)\rightarrow o
]

* Skill 是 **面向意图的证据工作流**：把多个 primitives 组合成可完成某类 evidence need 的流程；必要时插入少量 (C) 做结构化判别/抽取。
* 约束：编排逻辑本身确定性；所有节点输出可追溯；端到端产出 Observation（含 provenance）。

#### 4) Compiler LLM（离线）：候选生成器（只在 Foundry 出现）

记离线编译器为 (\mathcal{L}_{comp})。它从语义实例中提出候选能力与测试，但 **不直接决定收录**：

* 输入：semantic act instances 中的 ((\text{intent}, \rho))，尤其是 latent_tool_calls、grounding_ref、target_type、source_seg_ids；
* 输出：候选 (p) 或 (s) 的 `{spec, code, tests}`；
* 收录条件：完全由 **沙箱执行与 tests** 决定。

---

## 离线编译过程：EAC（Executable Artifact Compilation）循环

### 总：一句话概括

Foundry 执行一个同构循环：**语义实例挖掘能力需求 → 编译器生成候选（含测试）→ 沙箱执行门控 → 通过则入库，否则迭代修复或丢弃**。

### 分：形式化描述（可作为方法段落/Algorithm 1）

#### (A) 候选发现（Candidate Mining）

从语义实例集合 (\mathcal{A}) 中提取能力需求单元：
[
u = (\text{intent},\ \text{evidence_need},\ \rho)
]
其中 evidence_need 可由 latent_tool_calls.category/operation 与 grounding_ref 类型诱导得到。高频 (u) 对应优先编译的能力。

#### (B) Primitive 编译（生成最小可复用证据算子）

对每个候选 primitive 需求 (u)：

1. (\mathcal{L}_{comp}) 生成 (p) 的 spec、实现代码与测试用例；
2. 在沙箱中运行 tests：schema 校验、确定性/资源门控、失败码覆盖；
3. 通过则加入 (\mathcal{P})，并登记其 obs_type 与 provenance 规范。

#### (C) Skill 编译（组合覆盖 intent 的证据工作流）
对每个候选 skill 需求 (u)：

1. (\mathcal{L}_{comp}) 生成编排 DAG（调用若干 (p)，可插入少量 (c)）；
2. 生成端到端 tests（包含典型样本的回归用例）；
3. 通过则加入 (\mathcal{S})。

#### (D) 回归闭环（来自 Arena 的 failure/missing）

在线 Arena 的统计信号回流为离线测试增强：

* 高频 `failure_code` → 生成针对性回归测试；
* 高频 `status=missing` → 触发新增 primitive/skill 的候选需求。

> 这一闭环是“证据条件化策略行为”可持续扩展的关键：策略模型不必幻觉补齐证据，而是推动能力库扩张。
