...
### 3.3 预处理步骤（可复现）
1. **展平与对齐**：展开 `mining_results`，生成 `issue_id`，保留 `role/grounding_ref/strategic_intent/action/latent_tool_calls`。  
2. **grounding_ref → paper_span**：将 `grounding_ref` 解析为段落/页码/公式索引（失败则保留原值）。  
3. **规则归类**：基于 `strategic_intent` 与关键词（novelty / correctness / ablation / significance 等）做初步标注。  
4. **嵌入聚类**：  
   - 向量：`operation ⊕ target_type ⊕ outcome ⊕ action`（从 `latent_tool_calls` 提取）  
   - 聚类：HDBSCAN（主）+ KMeans（对照）  
5. **Issue 合并与类型稳定化**：输出 `issue_type` 与 `cluster_id`。  
...
1. `foundry/issue_mining/parse_mining_results.py`：展平 `analysis.mining_results` 为 issue 记录。输入 `data/raw/mining_results.jsonl`，输出 `data/interim/mining_flat.jsonl`。  
```bash
python foundry/issue_mining/parse_mining_results.py --in data/raw/mining_results.jsonl --out data/interim/mining_flat.jsonl
```
2. `foundry/issue_mining/link_grounding_ref.py`：将 `grounding_ref` 解析到 paper span。输入 `data/interim/mining_flat.jsonl` 与论文存档，输出 `data/interim/grounded_issues.jsonl`。  
```bash
python foundry/issue_mining/link_grounding_ref.py --in data/interim/mining_flat.jsonl --papers data/raw/papers --out data/interim/grounded_issues.jsonl
```
3. `foundry/issue_mining/cluster_issues.py`：按 `(operation, target_type, outcome, action)` 聚类并生成 `issue_type/cluster_id`。输入 `data/interim/grounded_issues.jsonl`，输出 `data/processed/issues.jsonl`。  
```bash
python foundry/issue_mining/cluster_issues.py --in data/interim/grounded_issues.jsonl --out data/processed/issues.jsonl
```
4. `foundry/ontology/build_ontology.py`：基于聚类结果生成 intent/evidence/issue 类型表。输入 `data/processed/issues.jsonl`，输出 `foundry/ontology/*.json`。  
```bash
python foundry/ontology/build_ontology.py --in data/processed/issues.jsonl --out foundry/ontology
```
5. `foundry/curation/extract_tool_candidates.py`：从 `latent_tool_calls` 抽取候选工具并判定可执行性。输入 `data/processed/issues.jsonl`，输出 `data/interim/tool_candidates.jsonl`。  
```bash
python foundry/curation/extract_tool_candidates.py --in data/processed/issues.jsonl --out data/interim/tool_candidates.jsonl
```
6. `foundry/curation/compile_skills.py`：将可执行候选编译为 skills，并更新 `skills/registry.json`。输入 `data/interim/tool_candidates.jsonl`，输出 `skills/library/*`。  
```bash
python foundry/curation/compile_skills.py --in data/interim/tool_candidates.jsonl --skills-dir skills/library --registry skills/registry.json
```
7. `foundry/curation/run_skills.py`：对 issue 执行技能，产出结构化 obs。输入 `data/processed/issues.jsonl` + `skills/registry.json`，输出 `data/interim/observations.jsonl`。  
```bash
...
```
8. `foundry/curation/curate_trajectories.py`：predicate 过滤并生成黄金轨迹。输入 `data/processed/issues.jsonl` + `data/interim/observations.jsonl`，输出 `data/processed/trajectories.jsonl`。  
```bash
python foundry/curation/curate_trajectories.py --issues data/processed/issues.jsonl --obs data/interim/observations.jsonl --out data/processed/trajectories.jsonl
```

### 4.6 Foundry 核心接口（JSON）
**ToolCandidate**（从 `latent_tool_calls` 生成）：  
```json
{
  "candidate_id": "GLWf2fq0bX#0003#0",
  "issue_id": "GLWf2fq0bX#0003",
  "tool_category": "Literature_Cross_Check",
  "operation": "Compare_with_Prior_Art",
  "target_type": "PSEC_vs_Prior_Work",
  "outcome": "...",
  "executability": "unknown|yes|no",
  "evidence_type": "Citation|Statistical|Symbolic|..."
}
```
