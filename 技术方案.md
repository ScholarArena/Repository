...
**流水线脚本（带检查）**
为了定位失败环节，提供分步脚本与量化检查：  
```bash
python scripts/pipeline/01_parse.py
python scripts/pipeline/02_grounding.py
python scripts/pipeline/03_cluster.py
python scripts/pipeline/04_ontology.py
python scripts/pipeline/05_threads.py
python scripts/pipeline/06_primitives.py
python scripts/pipeline/07_skills_and_trajectories.py
```
全量执行可用：
```bash
