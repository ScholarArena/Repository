...
```
3. `foundry/issue_mining/cluster_issues.py`：基于 embedding 聚类生成 `issue_type/cluster_id`。输入 `data/interim/grounded_issues.jsonl` + `data/raw/embeddings.npy`，输出 `data/processed/issues.jsonl`。  
```bash
python foundry/issue_mining/cluster_issues.py --in data/interim/grounded_issues.jsonl --embeddings data/raw/embeddings.npy --out data/processed/issues.jsonl
```
