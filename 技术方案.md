...
```
6. `foundry/issue_mining/build_issue_threads.py`：按 issue ontology 生成 `thread_id` 与索引（默认 `thread-by=ontology`）。输入 `data/processed/issues.jsonl`，输出 `data/processed/threads.jsonl` + `data/processed/thread_index.jsonl`。  
```bash
